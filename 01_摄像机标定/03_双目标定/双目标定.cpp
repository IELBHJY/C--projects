...
0 0  1
*/
Mat cameraMatrixR = (Mat_<double>(3, 3) << 1081.2088623, 0,       982.168945313,
											0,      1070.92443848, 516.604980469,
											0,		0,		 1);
Mat distCoeffR = (Mat_<double>(5, 1) << 0.0901803597808, -0.144965916872, -0.000159036761033, 0.00254778400995, 0);
/*
事先标定好的右相机的内参矩阵
fx 0 cx
0 fy cy
0 0  1
*/
Mat cameraMatrixL = (Mat_<double>(3, 3) << 373.207641602, 0,      255.502548218,
											0,      370.876403809, 208.960723877,
											0,		0,		 1);
Mat distCoeffL = (Mat_<double>(5, 1) << 0.110629461706,-0.415860265493, -0.00102956523187, -0.00315916305408, 0);
...
{
	Mat img;
	int goodFrameCount = 3;
	namedWindow("ImageL");
	namedWindow("ImageR");
	cout << "按Q退出 ..." << endl;
	while (goodFrameCount < frameNumber+3)
	{
		char filename[100];
		/*读取左边的图像*/
		sprintf_s(filename, "E:\\复杂机电\\双目标定图片\\rgb%d.bmp", goodFrameCount);
		rgbImageL = imread(filename,CV_LOAD_IMAGE_COLOR);
		cvtColor(rgbImageL, grayImageL, CV_BGR2GRAY);

		/*读取右边的图像*/
		sprintf_s(filename, "E:\\复杂机电\\双目标定图片\\ir%d.png", goodFrameCount);
